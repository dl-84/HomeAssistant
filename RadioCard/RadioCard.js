class RadioCard extends HTMLElement {
  ...
  static getStubConfig() {
    return {
      ...
      stations: [
        { name: "Absolut Top", url: "media-source://radio_browser/32bd882d-8d12-4a97-8058-f4af8485772b" },
      ],
      players: [],
    };
  }
  ...
  static get _css() {
    return `
      ...
      #play { background: #342C3E; }
      #play:hover:not(:disabled) { opacity: 0.85; }
      #play:active:not(:disabled) { opacity: 0.7; }
      #play:disabled { background: #504A58; color: #A8A2B0; cursor: not-allowed; }
      #pause { background: #342C3E; }
      #pause:hover:not(:disabled) { opacity: 0.85; }
      #pause:active:not(:disabled) { opacity: 0.7; }
      #pause:disabled { background: #504A58; color: #A8A2B0; cursor: not-allowed; }
      #reset { background: var(--error-color, #c00); border: none; color: white; }
      #reset:disabled { background: #504A58; color: #A8A2B0; cursor: not-allowed; }
      #reset-config { background: #342C3E; border: none; color: var(--primary-text-color); }
      #reset-config:disabled { background: #504A58; border: none; color: #A8A2B0; cursor: not-allowed; }
      ...
      .btn-row .stop-btn:disabled { background: #504A58; border: none; color: #A8A2B0; cursor: not-allowed; }
      .btn-row .btn-group-toggle { background: #342C3E; border: none; color: var(--primary-text-color); }
      .btn-row .btn-group-toggle:disabled { background: #504A58; border: none; color: #A8A2B0; cursor: not-allowed; }
      .btn-row .vol-down, .btn-row .vol-up { background: #342C3E; border: 1px solid #342C3E; color: var(--primary-text-color); }
      .btn-row .vol-down:disabled, .btn-row .vol-up:disabled { border: 1px solid #504A58; color: #A8A2B0; cursor: not-allowed; }
      .btn-row .vol-display { background: #342C3E; border: 1px solid #342C3E; color: var(--primary-text-color); font-weight: bold; cursor: default; }
      .btn-row .btn-pause { background: #342C3E; border: none; color: white; }
      .btn-row .btn-pause:disabled { background: #504A58; color: #A8A2B0; cursor: not-allowed; }
    `;
  }
  ...
  _buildPlayerRows() {
    ...
    players.forEach((p, i) => {
      ...
      const isPlaying = state && (state.state === "playing" || state.state === "paused");
      const isActivelyPlaying = state && state.state === "playing";
      const inGroup = state && state.attributes.group_members && state.attributes.group_members.length > 1;
      const isMaster = this._masterEntity !== null && this._masterEntity === p.entity;
      const isSlave = inGroup && !isMaster;
      const isGrouped = isSlave || this._groupedEntities.includes(p.entity);
      const isMasterWithSlaves = isMaster && (inGroup || this._groupedEntities.length > 0);
      const isInPaused = this._pausedEntities.includes(p.entity);
      const pauseBtnDisabled = (!isActivelyPlaying && !isInPaused) ? " disabled" : "";
      ...
      const volume = (state && state.attributes.volume_level != null)
        ? Math.round(state.attributes.volume_level * 100) : 50;

      let trackInfo;
      if (isPlaying) {
        trackInfo = (artist || title) ? (artist ? `${artist} — ${title}` : title) : "Wird gestartet...";
      ...
      div.innerHTML = `
        ...
        <div class="player-track">${trackInfo}</div>
        <div class="btn-row">
  ...
  _buildHTML(stationOptions, playerOptions, playDisabled, pauseDisabled, pauseIcon, stopDisabled, resetDisabled) {
    return `
  ...
  _bindEvents(isResumeMode) {
    this.shadowRoot.getElementById("station").addEventListener("change", (e) => {
    ...
    this.shadowRoot.getElementById("reset").addEventListener("click", () => {
      (this.config.players || []).forEach((p) => {
        this._hass.callService("media_player", "media_stop", { entity_id: p.entity });
      });
      this._masterEntity = null;
    ...
    this.shadowRoot.getElementById("reset-config").addEventListener("click", () => {
      this._masterEntity = null;
      this._masterAutoDetected = true;
      this._pausedEntities = [];
      this._resumingEntities = [];
      this._groupedEntities = [];
      (this.config.players || []).forEach((p) => {
        this._hass.callService("media_player", "unjoin", { entity_id: p.entity });
    ...
    this.shadowRoot.getElementById("play").addEventListener("click", () => {
      ...
      const playerInGroup = (playerState && playerState.attributes.group_members && playerState.attributes.group_members.length > 1)
        || this._groupedEntities.includes(player.entity);

      const startPlayback = () => {
        const currentVol = playerState && playerState.attributes.volume_level != null
          ? playerState.attributes.volume_level * 100 : 0;
        if (currentVol > 10) {
          this._hass.callService("media_player", "volume_set", {
      ...
      if (playerInGroup) {
        this._groupedEntities = this._groupedEntities.filter(e => e !== player.entity);
        this._hass.callService("media_player", "unjoin", { entity_id: player.entity });
    ...
    this.shadowRoot.getElementById("pause").addEventListener("click", () => {
      if (isResumeMode) {
        ...
        this._pausedEntities.forEach((entity) => {
          this._hass.callService("media_player", "media_play", { entity_id: entity });
        });
        this._pausedEntities = [];
      } else {
        this._pausedEntities = [];
        (this.config.players || []).forEach((p) => {
          ...
          if (s && s.state === "playing") {
            ...
            this._hass.callService("media_player", "media_pause", { entity_id: p.entity });
          }
        });
      }
      this.render();
    ...
    this.shadowRoot.querySelectorAll(".stop-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const player = this.config.players[idx];
        this._hass.callService("media_player", "media_stop", { entity_id: player.entity });
        if (this._masterEntity === player.entity) {
          this._masterEntity = null;
        }
        this.render();
      });
    ...
    this.shadowRoot.querySelectorAll(".btn-pause").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (this._pausedEntities.includes(player.entity)) {
          ...
          this._pausedEntities = this._pausedEntities.filter(e => e !== player.entity);
          this._hass.callService("media_player", "media_play", { entity_id: player.entity });
        } else {
          this._pausedEntities = [...this._pausedEntities, player.entity];
          this._hass.callService("media_player", "media_pause", { entity_id: player.entity });
        }
        this.render();
      });
    });
    ...
    this.shadowRoot.querySelectorAll(".btn-group-toggle").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (isMasterClick) {
          ...
          const haGroupMembers = ((state && state.attributes.group_members) || []).filter(e => e !== oldMaster);
          const newMasterEntity = this._groupedEntities[0] || haGroupMembers[0] || null;

          this._masterEntity = newMasterEntity;
          if (newMasterEntity) {
            this._groupedEntities = this._groupedEntities.filter(e => e !== newMasterEntity);
          }
          this._hass.callService("media_player", "unjoin", { entity_id: oldMaster });
          this._hass.callService("media_player", "media_stop", { entity_id: oldMaster });
          if (newMasterEntity) {
            setTimeout(() => {
              this._hass.callService("media_player", "media_play", { entity_id: newMasterEntity });
            }, 800);
          }
          ...
          if (this._masterEntity) {
            setTimeout(() => {
              this._hass.callService("media_player", "media_play", { entity_id: this._masterEntity });
            }, 800);
          }
        } else {
      ...
      btn.addEventListener("click", () => {
        ...
        dialog.innerHTML = `
          <style>
            ...
            .dl-vol-track { width: 100px; height: 220px; border-radius: 30px; background: #342C3E; position: relative; cursor: pointer; touch-action: none; user-select: none; overflow: hidden; }
            .dl-vol-fill { position: absolute; bottom: 0; left: 0; right: 0; background: #926BC7; border-radius: 0; pointer-events: none; }
            ...
            .dl-vol-preset.active { background: #342C3E; color: white; }
          </style>
          <div class="dl-vol-popup">
            <button class="dl-vol-close"><ha-icon icon="mdi:close"></ha-icon></button>
            <div class="dl-vol-name">${player.name}</div>
            <div class="dl-vol-value">${volume}</div>
            <div class="dl-vol-track">
              <div class="dl-vol-fill" style="height:${Math.min(volume, 25) / 25 * 100}%">
                <div class="dl-vol-handle"></div>
              </div>
            </div>
            <div class="dl-vol-presets">
              <button class="dl-vol-preset${volume === 5 ? " active" : ""}" data-val="5">5</button>
              <button class="dl-vol-preset${volume === 10 ? " active" : ""}" data-val="10">10</button>
              <button class="dl-vol-preset${volume === 15 ? " active" : ""}" data-val="15">15</button>
              <button class="dl-vol-preset${volume === 20 ? " active" : ""}" data-val="20">20</button>
            </div>
          </div>
        `;
        ...
        const updateUI = (val) => {
          fill.style.height = (val / 25 * 100) + "%";
          valueDisplay.textContent = val;
          dialog.querySelectorAll(".dl-vol-preset").forEach((p) => {
        ...
        const getVolFromY = (clientY) => {
          ...
          return Math.round(Math.min(1, Math.max(0, (rect.bottom - clientY) / rect.height)) * 25);
        };
        ...
        const onEsc = (e) => { if (e.key === "Escape") { e.stopImmediatePropagation(); e.preventDefault(); closePopup(); } };
        document.addEventListener("keydown", onEsc, { capture: true });
        dialog.addEventListener("cancel", (e) => { e.preventDefault(); closePopup(); });

        const closePopup = () => { dialog.close(); dialog.remove(); document.removeEventListener("keydown", onEsc, { capture: true }); };

        dialog.addEventListener("click", (e) => {
          if (!dialog.querySelector(".dl-vol-popup").contains(e.target)) closePopup();
        });
    ...
    this.shadowRoot.querySelectorAll(".vol-down, .vol-up").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const newVal = Math.min(25, Math.max(0, current + (isUp ? 1 : -1)));
  ...
  render() {
    ...
    const playDisabled = this._selectedStation === -1 || this._selectedPlayer === -1 ? " disabled" : "";
    ...
    const stopDisabled = (anyPlaying || anyPaused || this._pausedEntities.length > 0) ? "" : " disabled";
    ...
    this.shadowRoot.innerHTML = this._buildHTML(
      stationOptions, playerOptions, playDisabled, pauseDisabled, pauseIcon, stopDisabled, resetDisabled
    );
...
class RadioCardEditor extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    this._config = { stationLabel: "Sender", playerLabel: "Lautsprecher", stations: [], players: [] };
  }

  connectedCallback() {
    this._escHandler = (e) => {
      if (e.key !== "Escape" || document.querySelector(".pp-wrapper,.sp-wrapper,.dl-vol-wrapper")) return;
      const dlg = this._findHaEditDialog();
      if (!dlg) return;
      e.stopImmediatePropagation(); e.preventDefault();
      if (typeof dlg.closeDialog === "function") {
        // If we know no changes were made, reset HA's internal dirty flag so it
        // doesn't show the "Unsaved Changes" confirmation unnecessarily.
      ...
      (dlg.shadowRoot?.querySelector("ha-dialog") ?? dlg.shadowRoot?.querySelector("dialog"))
  ...
  _findHaEditDialog() {
    ...
    // querySelector does NOT pierce shadow roots, so walk each boundary manually.
    // HA shadow DOM path: document → home-assistant/#shadow → home-assistant-main/#shadow
    //   → ha-panel-lovelace or partial-panel-resolver/#shadow → hui-root/#shadow → dialog
    const ha = document.querySelector("home-assistant");
    const haR = ha?.shadowRoot;
    const main = haR?.querySelector("home-assistant-main");
    const mainR = main?.shadowRoot;
    const panel = mainR?.querySelector("ha-panel-lovelace") ?? mainR?.querySelector("partial-panel-resolver");
    const panelR = panel?.shadowRoot;
    const huiRoot = panelR?.querySelector("hui-root");
    const huiRootR = huiRoot?.shadowRoot;
    return q(document) ?? q(haR) ?? q(mainR) ?? q(panelR) ?? q(huiRootR) ?? null;
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .item.drag-over { outline: 2px solid #926BC7; border-radius: 4px; background: rgba(146,107,199,0.08); }
        ...
        input[type="text"]:focus { outline: none; border-color: #926BC7; }
  ...
  _showPlayerPicker() {
    ...
    const getRefRect = () => {
      ...
      let node = this.parentElement ?? (this.getRootNode() instanceof ShadowRoot ? this.getRootNode().host : null);
      while (node && node !== document.documentElement) {
        ...
        const r = node.getBoundingClientRect();
        // First ancestor significantly wider than editor but not full viewport = the dialog panel
        if (r.width > editorRect.width + 100 && r.width < window.innerWidth * 0.99 && r.height > 300) {
          return r;
        }
        node = node.parentElement ?? (node.getRootNode() instanceof ShadowRoot ? node.getRootNode().host : null);
      }
      return editorRect.width > 0
        ? { left: editorRect.left, top: editorRect.top - 60, width: editorRect.width, height: editorRect.height + 120 }
        : { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight };
    };
    const ref = getRefRect();
    ...
    dialog.style.cssText = "display:block;padding:0;margin:0;border:none;background:transparent;overflow:visible;max-width:none;max-height:none;pointer-events:none";
    dialog.innerHTML = `
      <style>
    ...
    const close = () => { dialog.close(); dialog.remove(); document.removeEventListener("keydown", onEsc, { capture: true }); };
    ...
    dialog.querySelectorAll(".pp-item").forEach(item => {
      item.addEventListener("click", () => {
        this._config.players.push({ entity: item.dataset.entity, name: item.dataset.name });
        this._render();
        this._fire();
        close();
      });
  ...
  _showStationPicker() {
    ...
    const getRefRect = () => {
      ...
      while (node && node !== document.documentElement) {
        ...
        const r = node.getBoundingClientRect();
        if (r.width > editorRect.width + 100 && r.width < window.innerWidth * 0.99 && r.height > 300) return r;
        node = node.parentElement ?? (node.getRootNode() instanceof ShadowRoot ? node.getRootNode().host : null);
      }
      return editorRect.width > 0
        ? { left: editorRect.left, top: editorRect.top - 60, width: editorRect.width, height: editorRect.height + 120 }
    ...
    dialog.innerHTML = `
      <style>
        ...
        .sp-search:focus { border-color: #926BC7; }
        ...
        .sp-spinner { width: 32px; height: 32px; border: 3px solid var(--divider-color, #444); border-top-color: #926BC7; border-radius: 50%; animation: sp-spin 0.8s linear infinite; }
    ...
    const renderList = (stations) => {
      const filtered = stations.filter(s => !added.has("media-source://radio_browser/" + s.stationuuid));
      ...
      list.querySelectorAll(".sp-item").forEach(item => {
        item.addEventListener("click", () => {
          this._config.stations.push({ name: item.dataset.name, url: item.dataset.url });
          this._render();
          this._fire();
          close();
        });
    ...
    const fetchStations = async (query = "") => {
      list.innerHTML = '<div class="sp-status"><div class="sp-spinner"></div></div>';
      try {
        const url = query
        ...
        list.innerHTML = '<div class="sp-status">Fehler beim Laden der Sender</div>';
      }
    };
    ...
    dialog.querySelector(".sp-search").addEventListener("input", (e) => {
      ...
      debounceTimer = setTimeout(() => fetchStations(e.target.value.trim()), 300);
    });
  }

  _bindEditorEvents() {
    const on = (sel, evt, fn) => this.shadowRoot.querySelectorAll(sel).forEach(el => el.addEventListener(evt, fn));

    this.shadowRoot.getElementById("station-label").addEventListener("change", (e) => {
      this._config.stationLabel = e.target.value; this._fire();
    });
    this.shadowRoot.getElementById("player-label").addEventListener("change", (e) => {
      this._config.playerLabel = e.target.value; this._fire();
    });

    on(".st-name", "change", (e) => { this._config.stations[+e.target.dataset.index].name = e.target.value; this._fire(); });
    on(".st-url", "change", (e) => { this._config.stations[+e.target.dataset.index].url = e.target.value; this._fire(); });
    on(".pl-entity", "change", (e) => { this._config.players[+e.target.dataset.index].entity = e.target.value; this._fire(); });
    on(".pl-name", "change", (e) => { this._config.players[+e.target.dataset.index].name = e.target.value; this._fire(); });
    ...
    on(".item[draggable]", "dragstart", (e) => {
      ...
      setTimeout(() => dragSrc && dragSrc.style.setProperty("opacity", "0.4"), 0);
    });
    on(".item[draggable]", "dragend", (e) => {
      e.currentTarget.style.removeProperty("opacity");
      this.shadowRoot.querySelectorAll(".item.drag-over").forEach(el => el.classList.remove("drag-over"));
      dragSrc = null;
    });
    on(".item[draggable]", "dragover", (e) => { e.preventDefault(); e.dataTransfer.dropEffect = "move"; });
    on(".item[draggable]", "dragenter", (e) => { e.currentTarget.classList.add("drag-over"); });
    on(".item[draggable]", "dragleave", (e) => { e.currentTarget.classList.remove("drag-over"); });
    on(".item[draggable]", "drop", (e) => {
      ...
      const section = dragSrc.dataset.dragSection;
      const srcIdx = +dragSrc.dataset.dragIndex;
      const dstIdx = +dst.dataset.dragIndex;
      const [moved] = this._config[section].splice(srcIdx, 1);
      this._config[section].splice(dstIdx, 0, moved);
      this._render(); this._fire();
    });

    this.shadowRoot.getElementById("add-station").addEventListener("click", () => {
      this._showStationPicker();
    });
    this.shadowRoot.getElementById("add-player").addEventListener("click", () => {
      if (this._hass) {
        this._showPlayerPicker();
      } else {
        this._config.players.push({ entity: "", name: "" }); this._render(); this._fire();
      }
    });
  }
}
